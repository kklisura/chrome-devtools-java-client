...
public interface Network {
   ...
   *
   * @param enabled Whether requests should be intercepted. If patterns is not set, matches all and
   *     resets any previously set patterns. Other parameters are ignored if false.
   */
  @Experimental
   ...
   * @param enabled Whether requests should be intercepted. If patterns is not set, matches all and
   *     resets any previously set patterns. Other parameters are ignored if false.
   * @param patterns URLs matching any of these patterns will be forwarded and wait for the
   *     corresponding continueInterceptedRequest call. Wildcards ('*' -> zero or more, '?' ->
   *     exactly one) are allowed. Escape character is backslash. If omitted equivalent to ['*']
   *     (intercept all).
   */
  @Experimental
  void setRequestInterceptionEnabled(
      @ParamName("enabled") Boolean enabled,
      @Optional @ParamName("patterns") List<String> patterns);
   ...
   *
   * @param interceptionId
   * @param errorReason If set this causes the request to fail with the given reason. Passing <code>
   *     Aborted</code> for requests marked with <code>isNavigationRequest</code> also cancels the
